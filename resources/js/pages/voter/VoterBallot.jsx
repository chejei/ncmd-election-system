      ...
      <div className="max-w-screen-xl mx-auto bg-white p-8 rounded-2xl shadow-lg">
        ...
        {currentStep === 1 && (
          <>
            ...
            <p className="text-gray-600 mb-3">
              Select your preferred candidates for each position listed on the ballot.
            </p>
            <ul className="list">
              <li>Review all positions carefully.</li>
              <li>Choose only up to the allowed number of candidates per position.</li>
              <li>Double-check your choices before moving forward.</li>
            </ul>
            <div className="step-wrapper mt-6">
            {positions
              .filter((position) => position.candidates && position.candidates.length > 0)
              .map((position) => (
                <div key={position.id} className="mb-8">
                  <h2 className="text-xl font-semibold text-gray-700 mb-4">
                    ...
                    {position.max_winners > 1 && (
                      <span className="text-sm text-gray-500">
                        (Select up to {position.max_winners})
                      </span>
                    )}
                  </h2>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    {position.candidates.map((candidate) => (
                      ...
                      >
                        <input
                          type={position.max_winner > 1 ? "checkbox" : "radio"}
                          ...
                          onChange={() =>
                            handleSelect(position.id, candidate.id, position.max_winners)
                          }
                          className="hidden"
                        />
                        <img
        ...
        {currentStep === 2 && (
          <div>
            ...
            <ul className="list">
              ...
              <li>If you need to make changes, go back and update your ballot.</li>
              ...
              <tbody>
                {positions
                  ...
                  .map((position) => {
                    ...
                    const selectedIds = Array.isArray(selections[position.id])
                      ? selections[position.id]
                      : [selections[position.id]].filter(Boolean);
                    ...
                    return (
                      <tr key={position.id} className="hover:bg-gray-50">
                        <td className="px-4 py-2 border-b">{position.title}</td>
                        <td className="px-4 py-2 border-b">
                          {selectedCandidates.map((c, i) => (
                            <span key={c.id}>
                              {c.full_name}
                              {i < selectedCandidates.length - 1 && <br />}
                            </span>
                          ))}
                        </td>
                      </tr>
        ...
        {currentStep === 3 && (
          <div>
            ...
            <p className="text-gray-600 mb-3">
              This is the last step before submitting your ballot. <br />
              To complete your voting process, please capture a clear photo of yourself using the camera provided.
            </p>
            <h4 className="mb-2 text-m font-semibold">Why this step is important:</h4>
            <ul className="list">
              <li><span className="font-semibold">Identity Verification</span> – ensures that the person submitting the vote is really you.</li>
              <li><span className="font-semibold">Prevents Fraud</span> – avoids duplicate or unauthorized voting.</li>
              <li><span className="font-semibold">Election Integrity</span> – helps maintain fairness, transparency, and trust in the voting process.</li>
            </ul>
            <h4 className="mb-2 text-m font-semibold">Guidelines for taking your photo:</h4>
            <ul className="list">
              <li> Position yourself in front of the camera with good lighting.</li>
              <li> Make sure your face is clearly visible (no hats, masks, or obstructions).</li>
              <li> Once you capture the photo and click submit your vote will be automatically recorded and finalized.</li>
            </ul>
            <h4 className="mb-2 text-m font-semibold">Important Notice:</h4>
            <ul className="list">
              <li> If your photo appears unclear, anomalous, or suspicious (e.g., blurry face, multiple people, altered images), your vote may not be considered valid and could be invalidated.</li>
              <li> Please ensure your photo is taken properly to avoid issues with verification.</li>
            </ul>
            <div className="step-wrapper mt-6">
               {!photo ? (
                <>
                ...
                <>
                  <img src={photo} alt="Captured" className="w-full h-auto rounded-lg object-cover" />
                  <div className="flex justify-center mt-2"> 
                    <button

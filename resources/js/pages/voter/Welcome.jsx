...
export default function WelcomePage() {
  ...
  useEffect(() => {
    ...
    const fetchVoter = async (voterId) => {
         ...
         Swal.fire("Error", "Failed to load voter details", "error").then(() => {
            navigate("/voter");
         });
         } finally{
            setLoading(false);
  ...
  return (
      ...
      <div className="flex items-center justify-center min-h-screen bg-gray-100 px-4">
        ...
        <h2 className="text-3xl font-bold text-gray-800">
          ...
          <p className="text-gray-600 mb-6">
            You are now logged in as a registered voter. Please read the following carefully before proceeding.
          </p>

          {/* Warning */}
          <div className="bg-red-100 border-l-4 border-red-500 text-red-700 px-4 py-2 mb-6 rounded">
            <p className="text-xs"><span className="font-semibold">Warning:</span> If you are not the rightful owner of this account, please <a  onClick={handleLogout} className="cursor-pointer text-blue-600 hover:underline">logout</a> immediately.</p>
          </div>
          ...
          <div className="bg-blue-50 p-4 rounded mb-6">
            ...
            <ul className="list-disc pl-6 text-gray-700 space-y-1">
              <li>Fill out the ballot prayerfully and wisely.</li>
              <li>Vote personally and freely, without anyone forcing you.</li>
              <li>Respect the integrity and confidentiality of the election process.</li>
            </ul>
          </div>
      ...
      <section id="guidelines" className="bg-white text-black gradiant-articles">
        <div className="max-w-screen-xl sm:px-7 px-4 py-10 lg:py-20 mx-auto">
          <div>
            <h2 className="text-xl font-bold  mb-3 sm:text-2xl">Voter Information</h2>
            <h3 className="text-xl font-bold leading-none text-gray-900">{voter.full_name || "-"}</h3>
            <div className="py-4 flex-1">
              <dl className="mb-2">
                <dt className="font-semibold text-gray-900 ">Email</dt>
                <dd className="text-gray-500 ">{voter.email || "-"}</dd>
              </dl>
              <dl className="mb-2">
                <dt className="font-semibold text-gray-900 ">Phone Number</dt>
                <dd className="text-gray-500 ">{voter.phone_number || "-"}</dd>
              </dl>
              <dl className="mb-2">
                <dt className="font-semibold text-gray-900 ">Church</dt>
                <dd className="text-gray-500 ">{voter.church_name || "-"}</dd>
              </dl>
            </div>
          </div>
          {voter.voted_photo && (
          ...
          <div>
            <h3 className="text-lg font-semibold text-gray-800 mb-2">My Ballot</h3>
            ...
            <thead>
              ...
              <tbody>
                {Object.entries(groupedPositions).map(([position, candidates], index) => (
                  <tr key={index}>
           ...
           <hr className="my-6 border-gray-200" />
           <div>
             <h3 className="text-lg font-semibold text-gray-800 mb-2">Photo Verification</h3>
             <img
                 src={voter.voted_photo ? `/storage/${voter.voted_photo}` : "/images/user-placeholder.png"}

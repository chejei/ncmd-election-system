...
export default function Candidate() {
  ...
  return (
    <>
    <section id="profile" className={`${candidate.political_color && '' || 'bg-gray-300' } text-black`} style={{ backgroundColor: candidate.political_color }}>
         <div className="max-w-screen-xl sm:px-7 px-4 mx-auto">
            <div className="grid grid-cols-2 items-center">
              ...
              <div className="px-5 py-10 bg-white w-full rounded my-20 mx-auto">
                  <h1 className="text-5xl font-bold mb-3 leading-6">
                    {candidate.full_name}<br/>
                    <span className="text-2xl">for {candidate.position_title}</span>
                  </h1>
                  <div className="mb-4">
                    {candidate.age && ( <p className="text-lg"><strong>Age:</strong> {candidate.age} years old.</p>)}
                    {candidate.email && ( <p className="text-lg"><strong>Email:</strong> {candidate.email }</p>)}
                    {candidate.address && ( <p className="text-lg"><strong>Address:</strong> {candidate.address}</p>)}
                  </div>
                  <div className="mb-4">
                    <h3 className="text-1xl font-bold mb-2">Education Background</h3>
                    {candidate.grade_year && ( <p className="text-lg"><strong>Grade/Year:</strong> {candidate.grade_year}</p>)}
                    {candidate.course_strand && ( <p className="text-lg"><strong>Course/Strand:</strong> {candidate.course_strand }</p>)}
                    {candidate.school && ( <p className="text-lg"><strong>School:</strong> {candidate.school}</p>)}
                  </div>
                  {candidate.occupation && candidate.company && (
                  <div className="mb-4">
                    <h3 className="text-1xl font-bold mb-2">Professional Information</h3>
                    {candidate.occupation && ( <p className="text-lg"><strong>Ocupation:</strong> {candidate.occupation}</p>)}
                    {candidate.company && ( <p className="text-lg"><strong>Company:</strong> {candidate.company }</p>)}
                  </div>
                  )}
              </div>
            </div>
    ...
    <section id="ministry" className="text-black  py-10 lg:py-20">
         <div className="max-w-screen-xl sm:px-7 px-4 mx-auto">
            <h2 className="text-3xl font-bold mb-3">Ministry Involvement</h2>
    ...
    <section id="qa" className="text-black  py-10 lg:py-20">
         <div className="max-w-screen-xl sm:px-7 px-4 mx-auto">
            <h2 className="text-3xl font-bold mb-3">Get to know them more</h2>
            {candidate.answers.map((answer, index) => (
              <div key={`accordion-${answer.id}-wrapper`}>
                <hr key={`hr-${answer.id}`} className="w-full lg:mt-10 md:mt-12 md:mb-8 my-8" />
                <Accordion key={`accordion-${answer.id}`}  title={answer.question_text} content={answer.answer} index={index} />
              </div>
            ))}
         </div>
    </section>

...
export default function Questions() {
   ...
   const fetchQuestions = (page = 1, search = '') => {
      setQuestions([]);
      axios.get(`/questions?page=${page}&search=${search}`)
         .then(response => {
   ...
   const handleDeleteSelected = () => {
      if (selectedIds.length === 0) {
         Swal.fire("No Selection", "Please select at least one question.", "info");
         return;
      }
      ...
      Swal.fire({
         ...
         if (result.isConfirmed) {
            try {
               await Promise.all(selectedIds.map(id => axios.delete(`/questions/${id}`)));
               fetchQuestions(1, searchTerm);
               setCurrentPage(1);
               setSelectedIds([]);
   ...
   const handleToggleEnable = async (item, newValue) => {
      try {
         await axios.put(`/questions/${item.id}`, { question_text: item.question_text, enable: newValue }); 
         setQuestions((prev) =>
            prev.map((q) =>

...
export default function ViewVoter() {
  ...
  const handleSend = async (method) => {
    try {
      const res = await axios.post(`/voters/${voterId}/send-pin`, { method });
      Swal.fire("Success", res.data.message || `PIN sent via ${method}`, "success");
    } catch (error) {
      Swal.fire(
        "Error",
        error.response?.data?.message || `Failed to send PIN via ${method}`,
        "error"
      );
    }
  };
  ...
  return (
    <div className="bg-white border border-gray-100 shadow-md p-6 rounded-md lg:col-span-2">
                  ...
                  <div className="py-4 flex-1">
                     <dl className="mb-2">
                      ...
                      <div className="flex flex-row items-center gap-2 mb-2 pin-notif">
                        <p className="text-sm italic">Request for pin code: </p>
                           <button 
                      ...
                      <div className="flex flex-row items-center gap-2 mb-2 pin-notif">
                        <p className="text-sm italic">Request for reset pin code: </p>
                           <button 
         ...
         <div className="flex justify-end mt-6">
          ...
          {!voter.voted_photo && (
            <button
              onClick={() => navigate(`/admin/voters/edit/${voterId}`)}

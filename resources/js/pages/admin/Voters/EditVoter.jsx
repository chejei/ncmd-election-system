...
export default function EditVoter() {
      ...
      try {
        ...
        if (voter.church_name) {
          const church = churchRes.data.find(
            (c) => c.name.toLowerCase() === voter.church_name.toLowerCase()
          );
          if (church) {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (res.status === 200) {
        Swal.fire("Success", "Voter updated successfully!", "success").then(() => {
          navigate("/admin/voters");
        });
      }
    } catch (error) {
      console.error("Error:", error);
      Swal.fire("Error", error.response?.data?.message || "Failed to update voter.", "error");
    }
  };

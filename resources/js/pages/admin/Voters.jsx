...
export default function Voters() {
   ...
   const fetchVoters = (page = 1, search = '') => {
      setVoters([]);
      axios.get(`/voters?page=${page}&search=${search}`)
         .then(response => {
   ...
   const handleDeleteSelected = () => {
      if (selectedIds.length === 0) {
         Swal.fire("No Selection", "Please select at least one voter.", "info");
         return;
      }
      ...
      Swal.fire({
         ...
         if (result.isConfirmed) {
            try {
               await Promise.all(selectedIds.map(id => axios.delete(`/voters/${id}`)));
               fetchVoters(1, searchTerm);
               setCurrentPage(1);
               setSelectedIds([]);

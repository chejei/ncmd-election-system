...
export default function Churches() {
   ...
   const fetchChurches = (page = 1, search = '') => {
      setChurches([]);
      axios.get(`/churches?page=${page}&search=${search}`)
         .then(response => {
   ...
   const handleDeleteSelected = () => {
      if (selectedIds.length === 0) {
         Swal.fire("No Selection", "Please select at least one church.", "info");
         return;
      }
      ...
      Swal.fire({
         ...
         if (result.isConfirmed) {
            try {
               await Promise.all(selectedIds.map(id => axios.delete(`/churches/${id}`)));
               fetchChurches(1, searchTerm);
               setCurrentPage(1);
               setSelectedIds([]);

                    ...
                    <div className="flex-1">
                        <div className="flex flex-col items-center justify-center">
                            <PieChart width={200} height={200}>
                                ...
                                outerRadius={100}
                                startAngle={90}
                                endAngle={90 - (360 * (totalVoted / registeredVoters))}
                                dataKey="value"
                                isAnimationActive={false}   
                                focusable={false}  
                                >
                            ...
                            <p className="text-sx mt-3 text-center text-gray-800">Election Returns transmitted<br/> {totalVoted} of {registeredVoters} Voters As of {formatDateTime(dateTime)}</p>
                        </div>
                    </div>
                </div>
                                    ...
                                    <ul className="mx-2">
                                        {ranked.map((cand, index) => {
                                            const percentage =
                                                ...
                                                : 0;
                                            return (
                                            <li key={cand.id} className="rounded-md inset-shadow-2xs shadow-md/10 p-6 mb-4 flex items-center gap-2">
                                                <span className="w-10 h-10 flex-none flex justify-center rounded-full text-gray-900 border-1 p-2">{ordinalSuffix(cand.rank)}</span>
                                                <div className="flex-1">
                                                    <span className="font-semibold text-xl mb-2">{cand.full_name} – {cand.vote_count} votes</span>
